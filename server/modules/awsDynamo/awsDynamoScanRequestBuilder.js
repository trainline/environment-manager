        ...
        if ({}.hasOwnProperty.call($data.filter, field)) {
          ...
          switch (field.toLowerCase()) {
            case DATE_FROM_FIELD:
              dateFrom = tryParseDate($data.filter[field], $data.dateField.format);
              if (!dateFrom) continue; // eslint-disable-line no-continue
              filters.push(new Condition.GreaterOrEqual($data.dateField.name).than(dateFrom));
              break;
            case DATE_TO_FIELD:
              dateTo = tryParseDate($data.filter[field], $data.dateField.format);
              if (!dateTo) continue; // eslint-disable-line no-continue
