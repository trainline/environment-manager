...
function buildUserDataByName(userDataTemplateName, userData) {
  return new Promise((resolve) => {
    renderer.render(userDataTemplateName, userData, content => {
      var encodedContent = new Buffer(content).toString("base64");
...
module.exports = {
  buildLinuxUserData: function (userData) {
  ...
  buildWindowsUserData: function (userData) {
    assert(userData, "Expected 'userData' argument not to be null");
    return buildUserDataByName(WINDOWS_USER_DATA_TEMPLATE, userData);
  },
  LINUX_USER_DATA_TEMPLATE: LINUX_USER_DATA_TEMPLATE,
  WINDOWS_USER_DATA_TEMPLATE: WINDOWS_USER_DATA_TEMPLATE,
};
